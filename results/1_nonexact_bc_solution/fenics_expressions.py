...
class UexExpr(MyUserExpression):
    def __init__(self, degree, domain, pb_considered):
        super().__init__(degree, domain)
        self.pb_considered = pb_considered
    
    def eval(self, value, x):
        value[0] = self.pb_considered.u_ex(dol, x, None)
        
class PExpr(MyUserExpression):
    def __init__(self, degree, domain, pb_considered):
        super().__init__(degree, domain)
        self.pb_considered = pb_considered
    
    def eval(self, value, x):
        value[0] = self.pb_considered.pert(dol, x, None)
...
class FExpr(MyUserExpression):
    ...
    def eval(self, value, x):
        value[0] = self.pb_considered.f(dol, x, None)