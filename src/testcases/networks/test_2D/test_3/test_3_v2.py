...
class Poisson_2D(pdes.AbstractPDEx):
    def __init__(self):
        self.problem = TestCase3(version=2)
...
def Run_laplacian2D(pde, new_training = False, largenet=False):
    ...
    file_name = current / "networks" / "test_2D" / "test_fe3_v2.pth"
    ...
    if new_training:
        trainer.train(epochs=5000, n_collocation=8000, n_bc_collocation=0, n_data=0)

    filename = current / "networks" / "test_2D" / "test_fe3_v2.png"
