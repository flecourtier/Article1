...
class Poisson_2D(pdes.AbstractPDEx):
    ...
    def bc_residual(self, w, x, mu, **kwargs):        
        ...
        u_x_ext = self.get_variables(w, "w_x", label=1)
        u_y_ext = self.get_variables(w, "w_y", label=1)
        n_x_e, n_y_e = x.get_normals(label=1)
        x1, x2 = x.get_coordinates(label=1)
        h_ext = self.problem.h_ext(torch, [x1,x2], mu)
        
        # plt.figure()
        # plt.quiver(x1.cpu().detach().numpy(), x2.cpu().detach().numpy(), n_x_e.cpu().detach().numpy(), n_y_e.cpu().detach().numpy(),label="n_ext")
        # # plt.scatter(x1.cpu().detach().numpy(), x2.cpu().detach().numpy(), c=u_x_ext.cpu().detach().numpy(),label="u_x_ext")
        # plt.colorbar()
        ...
        # plt.quiver(x1.cpu().detach().numpy(), x2.cpu().detach().numpy(), n_x_i.cpu().detach().numpy(), n_y_i.cpu().detach().numpy(),label="n_int")
        # plt.legend()
        # plt.show()
    ...
    def residual(self, w, x, mu, **kwargs):
        ...
        # plt.figure()
        # plt.scatter(x1.cpu().detach().numpy(), x2.cpu().detach().numpy(), c=(-(u_xx + u_yy)).cpu().detach().numpy(),label="u")
        # plt.colorbar()
        # plt.show()
...
def Run_laplacian2D(pde,training=False,plot_bc=False):
    ...
    file_name = current / "networks" / "test_fe4_v5.pth"
    # new_training = False
    new_training = True
    ...
    if training:
        trainer.train(epochs=10000, n_collocation=8000, n_bc_collocation=8000)
        # trainer.train(epochs=1, n_collocation=8000, n_bc_collocation=8000)

    filename = current / "networks" / "test_fe4_v5.png"
